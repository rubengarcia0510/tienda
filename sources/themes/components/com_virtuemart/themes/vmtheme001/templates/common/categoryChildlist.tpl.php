...
?>
<br/>
<table width="100%" cellspacing="0" cellpadding="0" style="margin:0px 0px 0px 1px; width:99%">
<?php
foreach( $categories as $category ) {
	if ($iCol == 1) { // this is an indicator wether a row needs to be opened or not
		echo "<tr>\n";
	} else {
		echo "<td class=\"listing-separator\">&nbsp;</td>\n";
	}
	?>
	...
	<td width="<?php echo $cellwidth ?>%" >
		...
		<?php vm_border_item_top(); ?>
         <a class="product_link" title="<?php echo $category["category_name"] ?>" href="<?php $sess->purl(URL."index.php?option=com_virtuemart&amp;page=shop.browse&amp;category_id=".$category["category_id"]) ?>"> 
			<?php
	...
	if ($iCol == $categories_per_row) { // If the number of products per row has been reached
		echo "</tr><tr><td class=\"listing-separator\">&nbsp;</td></tr>\n";
		$iCol = 1;
	}
	else {
		$iCol++;
